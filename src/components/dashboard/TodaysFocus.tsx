...
export default function TodaysFocus() {
  return (
    <View className="bg-white rounded-lg shadow-md p-6 mb-6">    
        ...
        {dashboardData.stats.map((item: any) => (
        <View
            key={item.id}
    ...
    {[dashboardData.todayFocus].map((focus, index) => (
      <View key={index} style={styles.focusCard}>
        <View style={styles.focusTop}>
          ...
          <View style={styles.focusQuestionsBadge}>
            ...
            <Text style={styles.focusQuestionsLabel}>
              questions
            </Text>
          </View>
        </View>
...
const styles: any = {
  ...
  // Stats Grid
    ...
    statCard: {
        ...
        padding: 14, minWidth: (width - 60) / 2,
        shadowColor: COLORS.cardShadow, shadowOffset: { width: 0, height: 2 },
        shadowOpacity: 1, shadowRadius: 8, elevation: 2,
    },
    statIconBg: { width: 32, height: 32, borderRadius: 8, alignItems: 'center', justifyContent: 'center', marginBottom: 8 },
  ...
  focusTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' },
  focusLabel: { fontSize: 10, fontWeight: '700', color: 'rgba(255,255,255,0.7)', letterSpacing: 1 },
  focusTitle: { fontSize: 18, fontWeight: '800', color: COLORS.white, marginTop: 4 },
  focusSubjectRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 4 },
  focusSubject: { fontSize: 13, color: 'rgba(255,255,255,0.85)', fontWeight: '600' },
  focusQuestionsBadge: {
  ...
  focusMetaText: { fontSize: 12, color: 'rgba(255,255,255,0.8)', flex: 1 },
};
